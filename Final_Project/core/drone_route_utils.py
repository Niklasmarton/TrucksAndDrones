...
def internal_truck_indices(truck_route):
    # Includes both depot endpoints. A drone may launch from the depot
    # (launch_idx=0) and/or land at the depot (land_idx=len-1); the cost
    # evaluator zeroes out hover-at-depot, so depot-endpoint trips are
    # legal and often cheap on far-depot instances.
    return list(range(0, len(truck_route)))
...
def build_drone_pair(
    ...
    last_idx = len(truck_route) - 1
    last_cust_idx = last_idx - 1
    ...
    def feasible_pairs(index_pool):
        ...
        for launch_idx in index_pool:
            for land_idx in index_pool:
                ...
                # Land-aware sync penalties (per cost-evaluator semantics):
                #  - drone-slow (truck waits for drone) delays subsequent
                #    t_arrival's, but the post-last-customer leg is excluded
                #    from the objective. So drone-slow is free at land=depot
                #    AND at land=last_customer.
                #  - drone-fast (drone hovers at land) consumes flight-range
                #    UNLESS land is the depot (drone_wait zeroed at depot).
                if land_idx == last_idx:
                    wait_excess = 0.0
                    mismatch = 0.0
                elif land_idx == last_cust_idx:
                    wait_excess = 0.0
                    mismatch = max(0.0, truck_time - trip)
                else:
