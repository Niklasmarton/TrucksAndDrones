...
def _search_budget(truck_len):
    # cap candidate checks but stay broad enough on bigger instances
...
def _removal_gain(truck, start, seg_len, truck_times):
    ...
    path_cost = truck_times[a][truck[start]]
    for k in range(start, start + seg_len - 1):
        path_cost += truck_times[truck[k]][truck[k + 1]]
    path_cost += truck_times[truck[start + seg_len - 1]][b]
    return path_cost - truck_times[a][b]
...
def _insertion_delta(reduced_truck, ins_idx, segment, removal_gain, truck_times):
    ...
    ins_cost = truck_times[prev_node][segment[0]]
    for k in range(len(segment) - 1):
        ins_cost += truck_times[segment[k]][segment[k + 1]]
    ins_cost += truck_times[segment[-1]][next_node]
    ins_cost -= truck_times[prev_node][next_node]
    ...
    for seg_len in seg_lengths:
        ...
        for start in range(1, n - seg_len):
            ...
            for ins_idx in range(1, len(reduced)):
                # putting it back at the original index = same route
                if ins_idx == start:
                    continue
...
# Or-opt segment relocation, length 1..3
